...
export const registerTryOnEvents = (socket: Socket) => {
   ...
   * }
   */
  socket.on("tryon_request", async (data: any) => {
    try {
      const { type, id, modelImage, kioskId, token } = data;

      if (!type || !id || !kioskId || !token) {
        socket.emit("tryon_failed", { error: "Missing required fields: type, id, kioskId, or token" });
        return;
      }
      ...
      logger.info(`[Socket] User ${userId} requested try-on for ${type} ${id} on kiosk ${kioskId}`);
      ...
      if (type === "garment") {
        ...
        socket.emit("tryon_failed", { error: "Invalid try-on type. Must be 'garment' or 'outfit'" });
        return;
      }
      ...
      logger.error(`[Socket] Try-on request failed: ${err.message}`);
      socket.emit("tryon_failed", { error: err.message || "Failed to start try-on process" });
    }
  });
};
